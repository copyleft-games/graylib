...
*** GrlFillRule
:PROPERTIES:
:CUSTOM_ID: grlfillrule
:END:
Fill rule for =grl_image_fill_path=. Controls how self-intersecting or nested subpaths determine the filled region.
...
*** GrlImageBlendMode
:PROPERTIES:
:CUSTOM_ID: grlimageblendmode
:END:
Per-image CPU blend mode for =grl_image_draw_*= primitives. Distinct from =GrlBlendMode= (which controls GPU/texture blending). All modes except =GRL_IMAGE_BLEND_REPLACE= require an =R8G8B8A8= image; on other formats drawing falls back to =GRL_IMAGE_BLEND_REPLACE=.
...
*** GrlImageColorSpace
:PROPERTIES:
:CUSTOM_ID: grlimagecolorspace
:END:
Colour space in which =grl_image_draw_*= compositing math is performed, set per image with =grl_image_set_blend_color_space()=. Controls only the /blended/ modes; =GRL_IMAGE_BLEND_REPLACE= always overwrites and is unaffected.
...
*** GrlPorterDuffOp
:PROPERTIES:
:CUSTOM_ID: grlporterduffop
:END:
Porter-Duff compositing operator for =grl_image_composite()=. These are the twelve standard operators from the 1984 paper /Compositing Digital Images/ by Thomas Porter and Tom Duff. The operation formula for each pixel is:

#+begin_quote
=out_premul = Fa × src_premul + Fb × dst_premul= =out_alpha  = Fa × sa + Fb × da=
...
#+begin_quote
*Alias note:* =GRL_PORTER_DUFF_DST_OVER= is called =DEST_OVER= in some other APIs (e.g. the HTML Canvas 2D specification); the two names refer to the identical operation.
...
*** GrlGifQuantizer
:PROPERTIES:
:CUSTOM_ID: grlgifquantizer
:END:
Palette-building strategy for =GrlGifWriter= (see =grl_image_set...= setters on the writer). The default is byte-for-byte compatible with the original encoder.
...
** Touch/Gesture
:PROPERTIES:
:CUSTOM_ID: touchgesture
:END:
*** GrlGesture
:PROPERTIES:
:CUSTOM_ID: grlgesture
:END:
...
** Material Maps
:PROPERTIES:
:CUSTOM_ID: material-maps
:END:
*** GrlMaterialMapType
:PROPERTIES:
:CUSTOM_ID: grlmaterialmaptype
:END:
Material map types for PBR rendering. See [[file:models.org][Models]] for details.
...
:PROPERTIES:
:CUSTOM_ID: rlgl-low-level-opengl
:END:
*** GrlRlglDrawMode
:PROPERTIES:
...
:PROPERTIES:
:CUSTOM_ID: grlrlglframebufferattachtype
:END:
...
:PROPERTIES:
:CUSTOM_ID: grlrlglframebuffertextype
:END:
...
:PROPERTIES:
:CUSTOM_ID: software-renderer-rlsw
:END:
These enumerations configure the headless CPU rasterizer. See [[file:software-renderer.org][Software Renderer]] for the full API. The numeric values mirror the underlying =rlsw= =SW*= enumerators (themselves OpenGL token values), so they can be cast straight across the ABI boundary.

*** GrlSwState
:PROPERTIES:
:CUSTOM_ID: grlswstate
:END:
Toggleable rasterizer state capabilities, used with =grl_software_renderer_enable()= and =grl_software_renderer_disable()=.
...
*** GrlSwBuffer
:PROPERTIES:
:CUSTOM_ID: grlswbuffer
:END:
Framebuffer clear bitmask flags, combined with bitwise OR and passed to =grl_software_renderer_clear()=.
...
*** GrlSwMatrixMode
:PROPERTIES:
:CUSTOM_ID: grlswmatrixmode
:END:
Selects which matrix stack subsequent matrix operations affect, used with =grl_software_renderer_matrix_mode()=.
...
:CUSTOM_ID: grlswface
:END:
Selects which polygon faces are culled, used with =grl_software_renderer_cull_face()=.
...
*** GrlSwFormat
:PROPERTIES:
:CUSTOM_ID: grlswformat
:END:
Pixel component layout, used by =grl_software_renderer_tex_image_2d()= and the pixel-readback functions.
...
:CUSTOM_ID: grlswtype
:END:
Per-component data type, used by =grl_software_renderer_tex_image_2d()= and the pixel-readback functions.
